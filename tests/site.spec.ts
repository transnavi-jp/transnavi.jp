...
test('対応タグは否定文（「〜を行わずに」）を拾わない', async ({ page }) => {
  // ノリス美容クリニックの記載は「豊胸術を行わずに女性化が期待できる」＝豊胸は“しない”
  // 文脈なので、豊胸タグは付かない。
  await page.goto('/clinics/hrt-osaka-norris/');
  const norrisTags = page.locator('.clinic-tags');
  if (await norrisTags.count()) {
    expect(await textNoRuby(norrisTags)).not.toContain('豊胸');
  }
