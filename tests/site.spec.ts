...
test('知る・学ぶは記事をテーマ別に分け、タグと主要ページへの導線を持つ', async ({ page }) => {
  ...
  for (const href of ['/map/', '/resources/', '/library/', '/reference/']) {
    await expect(page.locator(`.topic-card-link[href="${href}"]`)).toBeVisible();
  }

  // 記事カードは話題のタグをカード内に表示し、タグは /tags/ をたどれる。
  const tagChips = page.locator('.topic-card .topic-tags a.page-tag[href^="/tags/"]');
  expect(await tagChips.count()).toBeGreaterThanOrEqual(20);

  // カード本体（大きなリンク）とタグ（独立リンク）は別々に押せる。
  const card = page.locator('.topic-card-link[href="/hrt-effects/"]');
